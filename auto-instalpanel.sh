...
# ============================================================
# PREMIUM UI THEME (colors, icons, box drawing)
# ============================================================
if [ -t 1 ]; then
    C_RESET=$'\033[0m'
    C_BOLD=$'\033[1m'
    ...
    C_BLUE=""; C_GREEN=""; C_RED=""; C_MAGENTA=""; C_GRAY=""
fi

BOX_W=64
ICON_OK="${C_GREEN}✔${C_RESET}"
ICON_INFO="${C_CYAN}➜${C_RESET}"
ICON_WARN="${C_GOLD}⚠${C_RESET}"
ICON_ERR="${C_RED}✖${C_RESET}"

_repeat() {
    # Usage: _repeat CHAR COUNT
    printf "%0.s$1" $(seq 1 "$2" 2>/dev/null)
}

hr() { printf "%s\n" "${C_CYAN}$(_repeat "─" "$BOX_W")${C_RESET}"; }
hr_gold() { printf "%s\n" "${C_GOLD}$(_repeat "═" "$BOX_W")${C_RESET}"; }

box_top()    { printf "%s\n" "${C_GOLD}╔$(_repeat "═" $((BOX_W-2)))╗${C_RESET}"; }
box_bottom() { printf "%s\n" "${C_GOLD}╚$(_repeat "═" $((BOX_W-2)))╝${C_RESET}"; }
box_line() {
    ...
    [ "$pad" -lt 0 ] && pad=0
    local rpad=$(( inner - ${#text} - pad ))
    printf "%s║ %s%s%s%s%s ║%s\n" "$C_GOLD" "$(_repeat " " "$pad")" "$color" "$text" "$C_RESET$C_GOLD" "$(_repeat " " "$rpad")" "$C_RESET"
}
box_line_left() {
    # Usage: box_line_left "text" [color]
    local text="$1"
    local color="${2:-$C_RESET}"
    local inner=$((BOX_W-4))
    local rpad=$(( inner - ${#text} ))
    [ "$rpad" -lt 0 ] && rpad=0
    printf "%s║ %s%s%s%s ║%s\n" "$C_GOLD" "$color" "$text" "$C_RESET" "$(_repeat " " "$rpad")" "$C_RESET"
}

section_header() {
    # Usage: section_header "JUDUL SEKSI"
    local title="$1"
    local line="${C_BLUE}── ${C_BOLD}${title}${C_RESET}${C_BLUE} $(_repeat "─" $((BOX_W - 5 - ${#title})))${C_RESET}"
    printf "%s\n" "$line"
}

premium_banner() {
    # Usage: premium_banner "LINE1" ["LINE2"] ["LINE3"]
    echo
    box_top
    box_line_left "$(_repeat " " $BOX_W)" >/dev/null 2>&1 || true
    local i=1
    for line in "$@"; do
        if [ "$i" -eq 1 ]; then
            box_line "$line" "${C_BOLD}${C_GOLD}"
        else
            box_line "$line" "${C_CYAN}"
        fi
        i=$((i+1))
    done
    box_bottom
...
die() {
    echo
    hr
    printf "%s %s%s%s\n" "$ICON_ERR" "$C_RED$C_BOLD" "GAGAL: $*" "$C_RESET"
    hr
    echo
    exit 1
}
...
provision_node_via_api() {
    echo
    section_header "⚙  AUTO PROVISION NODE (API)"
    echo
...
' "$cfg_body" "/etc/pterodactyl/config.yml"
    ...
    echo
    box_top
    box_line "✔  AUTO PROVISIONING NODE SELESAI  ✔" "${C_BOLD}${C_GREEN}"
    box_bottom
    printf "  %s Location ID : %s%s%s\n" "$ICON_OK" "$C_CYAN" "$LOCATION_ID" "$C_RESET"
    printf "  %s Node ID     : %s%s%s\n" "$ICON_OK" "$C_CYAN" "$NODE_ID" "$C_RESET"
    printf "  %s Node FQDN   : %s%s%s\n" "$ICON_OK" "$C_CYAN" "$NODE_DOMAIN" "$C_RESET"
    printf "  %s Memory      : %s%s MB (overallocate %s%%)%s\n" "$ICON_OK" "$C_CYAN" "$NODE_MEMORY" "$NODE_MEMORY_OVERALLOCATE" "$C_RESET"
    printf "  %s Disk        : %s%s MB (overallocate %s%%)%s\n" "$ICON_OK" "$C_CYAN" "$NODE_DISK" "$NODE_DISK_OVERALLOCATE" "$C_RESET"
    hr_gold
    echo
...
box_top
box_line_left ""
box_line "★ PTERODACTYL AUTO INSTALLER ★" "${C_BOLD}${C_GOLD}"
box_line "L U X X Y   A S T R A" "${C_MAGENTA}"
box_line_left ""
box_line "${C_DIM}LXST Auto Installer • Premium Edition v1${C_RESET}" "${C_GRAY}"
box_line_left ""
box_bottom
echo
section_header "INSTALL"
printf "  ${C_GOLD}[1]${C_RESET}  Install Panel saja\n"
printf "  ${C_GOLD}[2]${C_RESET}  Install Panel + Wings\n"
printf "  ${C_GOLD}[3]${C_RESET}  Install Wings saja\n"
printf "  ${C_GOLD}[4]${C_RESET}  Install Database saja\n"
printf "  ${C_GOLD}[5]${C_RESET}  Install phpMyAdmin saja\n"
printf "  ${C_GOLD}[6]${C_RESET}  Install Database + phpMyAdmin\n"
hr
section_header "MANAGE"
printf "  ${C_GOLD}[7]${C_RESET}  Ganti Subdomain Panel\n"
printf "  ${C_GOLD}[11]${C_RESET} Auto Create Node/Location ${C_DIM}(Panel & Wings sudah terinstall)${C_RESET}\n"
hr
section_header "UNINSTALL"
printf "  ${C_RED}[8]${C_RESET}  Uninstall Panel\n"
printf "  ${C_RED}[9]${C_RESET}  Uninstall Wings/Node\n"
printf "  ${C_RED}[10]${C_RESET} Uninstall Panel + Wings/Node\n"
hr_gold
echo

while true; do
    printf "%s" "${C_BOLD}${C_CYAN}[?]${C_RESET} Pilih opsi ${C_DIM}[1-11]${C_RESET}: "
    read -r MODE
...
premium_banner "⚙  MEMULAI PROSES INSTALASI  ⚙" "Mohon tunggu, jangan tutup terminal..."
...
if [ "$EXPECT_STATUS" -ne 0 ]; then
    echo
    box_top
    box_line "✖  INSTALASI GAGAL  ✖" "${C_BOLD}${C_RED}"
    box_bottom
    printf "  %s Exit code : %s%s%s\n" "$ICON_ERR" "$C_BOLD" "$EXPECT_STATUS" "$C_RESET"
    printf "  %s Log file  : %s%s%s\n" "$ICON_ERR" "$C_DIM" "$LOG_FILE" "$C_RESET"
    echo
    exit "$EXPECT_STATUS"
fi
...
box_top
box_line_left ""
box_line "✔  I N S T A L A S I   S E L E S A I  ✔" "${C_BOLD}${C_GREEN}"
box_line_left ""
box_bottom
echo
...
if [ -n "$PANEL_DOMAIN" ]; then
    printf "  %s Panel     : %s%shttps://%s%s\n" "$ICON_OK" "$C_BOLD" "$C_CYAN" "$PANEL_DOMAIN" "$C_RESET"
fi

if [ -n "$NODE_DOMAIN" ]; then
    printf "  %s Node      : %s%s%s%s\n" "$ICON_OK" "$C_BOLD" "$C_CYAN" "$NODE_DOMAIN" "$C_RESET"
fi

if [ -n "$DBHOST_USERNAME" ]; then
    printf "  %s DB User   : %s%s%s%s\n" "$ICON_OK" "$C_BOLD" "$C_CYAN" "$DBHOST_USERNAME" "$C_RESET"
    printf "  %s DB Host   : %s127.0.0.1:3306%s\n" "$ICON_OK" "$C_CYAN" "$C_RESET"
fi
...
case "$MODE" in
    1) printf "  %s %sPanel saja selesai dipasang.%s\n" "$ICON_OK" "$C_GREEN" "$C_RESET" ;;
    2)
        printf "  %s %sPanel + Wings selesai dipasang.%s\n" "$ICON_OK" "$C_GREEN" "$C_RESET"
        printf "  %s %sWings menunggu config.yml dari Node Panel.%s\n" "$ICON_INFO" "$C_DIM" "$C_RESET"
        ;;
    3)
        printf "  %s %sWings saja selesai dipasang.%s\n" "$ICON_OK" "$C_GREEN" "$C_RESET"
        printf "  %s %sWings menunggu config.yml dari Node Panel.%s\n" "$ICON_INFO" "$C_DIM" "$C_RESET"
        ;;
esac

hr
printf "  %s Log lengkap : %s%s%s\n" "$ICON_INFO" "$C_DIM" "$LOG_FILE" "$C_RESET"
hr_gold
printf "%s\n" "${C_MAGENTA}${C_BOLD}      ★ LUXXY ASTRA — PTERODACTYL AUTO INSTALLER ★${C_RESET}"
echo
