...
auto_generate_panel_api_key() {
    # Bikin Application API Key otomatis lewat "php artisan tinker" langsung di
    # server Panel (tanpa perlu login manual atau buka dashboard). Hanya bisa
    # jalan kalau Panel ada di server yang sama (MODE 2) karena butuh akses
    # filesystem/database Panel secara langsung.
    ...
    local tinker_file="/tmp/pterodactyl-autokey-$$.php"
    cat > "$tinker_file" <<'PHP_EOF'
<?php
try {
    ...
    if (!$user || !$user->root_admin) {
        echo "AUTOKEY_FAIL:no_admin_user\n";
        return;
    }
    ...
    echo "AUTOKEY_OK:" . $key->identifier . $plain . "\n";
} catch (\Throwable $e) {
    echo "AUTOKEY_FAIL:" . $e->getMessage() . "\n";
}
PHP_EOF

    local output
    output="$(cd /var/www/pterodactyl && AUTOKEY_ADMIN="$ADMIN_USERNAME" php artisan tinker "$tinker_file" 2>/dev/null)"
    rm -f "$tinker_file"
