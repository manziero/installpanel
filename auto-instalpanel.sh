...
if [ "$MODE" = "2" ] || [ "$MODE" = "3" ]; then
    ...
    if [ "$MODE" = "2" ]; then
        ...
        info "Node subdomain (otomatis, sama dengan Panel): $NODE_DOMAIN"
    else
        while true; do
...
if [ "$MODE" = "2" ] || [ "$MODE" = "3" ]; then
    ...
    echo
    echo "Kalau diaktifkan, script akan otomatis:"
    ...
    echo
    echo "Butuh Application API Key dari Panel"
    echo "(Panel > Admin > Application API > Create New, izin minimal: Locations & Nodes = Read + Write)"
    echo
