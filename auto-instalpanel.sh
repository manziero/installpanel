...
#
# Wrapper untuk pterodactyl-installer v1.3.0
# ============================================================
...
if ! command -v curl >/dev/null 2>&1; then
    ...
    info "curl belum tersedia. Menginstall curl..."
...
if ! command -v expect >/dev/null 2>&1; then
    ...
    info "expect belum tersedia. Menginstall expect..."
...
if [ "$MODE" = "3" ]; then
    ...
    while true; do
        ...
        read -r -p "Email Let's Encrypt: " ADMIN_EMAIL
...
curl -fsSL \
    ...
    "$UPSTREAM_URL" \
    -o "$TMP_INSTALLER" || \
    die "Gagal mengambil installer upstream."
...
if ! /bin/bash -n "$TMP_INSTALLER"; then
    ...
    die "Syntax installer upstream tidak valid."
...
expect {
    ...
    -re {.*Do you want to configure MySQL to be accessed externally\? \(y/N\): *$} {
    ...
    # ========================================================
    # GENERIC HTTPS CONFIRMATION
    # ========================================================

    -re {.*HTTPS request.*\(y/N\): *$} {
